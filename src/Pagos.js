...
export default function Pagos() {
  ...
  const cargarMas = async () => {
    ...
    } catch { setError("Error cargando más familias."); }
  ...
  const cargarCompleto = async () => {
    ...
    if (camion) params.camion = camion;
    if (diaFiltro) params.dia = diaFiltro;
    const res = await axios.get(`${API_URL}/resumen-pagos-completo`, { params });
  ...
  const exportarExcel = async () => {
    try {
      setExportando(true);
      const todas = await cargarCompleto();
      const filas = [];
      const titularRows = []; // índices de filas titular para negrita

      // Encabezado
      filas.push(["Jefe de Hogar / Grupo Familiar", "Camión", "Litros", "Personas", "Entregas", "Cobro ($)", "Pagado ($)", "Deuda ($)", "Estado"]);
      ...
      ws["!cols"] = [{ wch: 46 },{ wch: 10 },{ wch: 11 },{ wch: 10 },{ wch: 11 },{ wch: 14 },{ wch: 13 },{ wch: 13 },{ wch: 13 }];

      // Estilo encabezado
      ["A","B","C","D","E","F","G","H","I"].forEach(col => {
        const ref = `${col}1`;
        if (ws[ref]) ws[ref].s = { font: { bold: true, color: { rgb: "FFFFFF" } }, fill: { fgColor: { rgb: "0F4C81" } }, alignment: { horizontal: "center" } };
      });

      // Titulares en negrita con fondo azul claro
      titularRows.forEach(rowIdx => {
        const xlsxRow = rowIdx + 1;
        ["A","B","C","D","E","F","G","H","I"].forEach(col => {
          const ref = `${col}${xlsxRow}`;
          if (ws[ref]) ws[ref].s = { font: { bold: true }, fill: { fgColor: { rgb: "EFF6FF" } } };
        });
      });

      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, `Pagos ${MESES[mes]} ${anio}`);
      ...
      const ws2 = XLSX.utils.aoa_to_sheet([
        ...
        ["Morosos",          todas.filter(f => f.estado === "moroso").length],
        ["Sin entregas",     todas.filter(f => f.estado === "sin_entregas").length],
        [],
        ["Total cobrado",    todas.reduce((s, f) => s + (f.cobro_calculado||0), 0)],
        ["Total pagado",     todas.reduce((s, f) => s + (f.pagado||0), 0)],
        ["Total deuda",      todas.reduce((s, f) => s + (f.deuda||0), 0)],
      ]);
      ...
      alert("Error al exportar Excel"); console.error(e);
    ...
    try {
      ...
      if (resumenKpis) {
        [
          ...
          doc.roundedRect(x, 24, 52, 12, 2, 2, "F");
          doc.setFontSize(6.5); doc.setTextColor(100); doc.text(k.label, x + 3, 29);
          doc.setFontSize(9); doc.setTextColor(15, 76, 129); doc.text(k.val, x + 3, 35);
        });
      }
      ...
      todas.forEach(f => {
        const estadoLabel = f.estado === "pagado" ? "Pagado" : f.estado === "moroso" ? "Moroso" : "Sin entregas";
        bodyRows.push([f.nombre, f.camion, `${(f.litros||0).toLocaleString()} L`, f.personas, f.entregas_mes, `$${(f.cobro_calculado||0).toLocaleString()}`, `$${(f.pagado||0).toLocaleString()}`, `$${(f.deuda||0).toLocaleString()}`, estadoLabel]);
      ...
      doc.autoTable({
        ...
        columnStyles: {
          0: { cellWidth: 68 }, 1: { cellWidth: 15 }, 2: { cellWidth: 17 },
          3: { cellWidth: 13 }, 4: { cellWidth: 16 }, 5: { cellWidth: 22 },
        ...
        didParseCell: (data) => {
          ...
          if (!meta) return;
          if (meta.esTitular) {
            data.cell.styles.fillColor  = [224, 242, 254]; // azul claro
            data.cell.styles.fontStyle  = "bold";
            data.cell.styles.fontSize   = 8;
            if (data.column.index === 8) {
              if (meta.estado === "pagado")      data.cell.styles.textColor = [22, 163, 74];
              else if (meta.estado === "moroso") data.cell.styles.textColor = [220, 38, 38];
      ...
      alert("Error al exportar PDF"); console.error(e);
    } finally { setExportando(false); }
  };
  ...
  const abrirFamilia = async (fid) => {
    try {
      ...
      setFamiliaActual(res.data); setVista("familia");
  ...
  const registrarPago = async () => {
    ...
    try {
      await axios.post(`${API_URL}/pagos`, {
        jefe_id: modalPago.id || familiaActual?.id, anio, mes,
        monto: parseFloat(formPago.monto), forma_pago: formPago.forma_pago, observacion: formPago.observacion,
      });
      setModalPago(null); setFormPago({ monto: "", forma_pago: "efectivo", observacion: "" });
      cargarResumen();
  ...
  const abrirEditarFamilia = () => {
    setFormFamilia({ nombre: familiaActual.nombre, camion: familiaActual.camion, litros: String(familiaActual.litros), telefono: familiaActual.telefono || "" });
  ...
  const guardarFamilia = async () => {
    ...
    try {
      ...
      setModalEditarFamilia(false); abrirFamilia(familiaActual.id); cargarResumen();
  ...
  const guardarResidente = async () => {
    ...
    try {
      if (editResidente) { await axios.put(`${API_URL}/residentes/${editResidente}`, formResidente); }
      else { await axios.post(`${API_URL}/familias/${modalResidente}/residentes`, formResidente); }
    ...
    return (
      <div className="main-container fade-in">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 20 }}>
          {[{ label:"Camión",val:familiaActual.camion},{ label:"Litros/sem",val:`${familiaActual.litros?.toLocaleString()} L`},{ label:"Personas",val:`${familiaActual.personas} persona${familiaActual.personas!==1?"s":""}`},{ label:"Teléfono",val:familiaActual.telefono||"—"}].map((k,i)=>(
            <div key={i} style={sKpi}><div style={{fontSize:11,color:"#64748b"}}>{k.label}</div><div style={{fontSize:18,fontWeight:700}}>{k.val}</div></div>
          ))}
        </div>
        <div style={{ background:"#fff",borderRadius:14,padding:20,marginBottom:20,boxShadow:"0 2px 12px rgba(0,0,0,0.06)" }}>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12 }}>
            {[{ label:"Entregas del mes",val:datos.entregas_mes},{ label:"Total calculado",val:`$${datos.cobro_calculado?.toLocaleString()}`},{ label:"Pagado",val:`$${datos.pagado?.toLocaleString()}`,color:"#16a34a"},{ label:"Deuda",val:`$${datos.deuda?.toLocaleString()}`,color:datos.deuda>0?"#dc2626":"#16a34a"}].map((k,i)=>(
              <div key={i} style={sKpi}><div style={{fontSize:11,color:"#64748b"}}>{k.label}</div><div style={{fontSize:20,fontWeight:700,color:k.color||"#0f172a"}}>{k.val}</div></div>
            ))}
          </div>
          <button onClick={()=>setModalPago({...datos,id:familiaActual.id,nombre:familiaActual.nombre})} style={{...sBtn("#1d4ed8","#fff"),marginTop:14,width:"100%",padding:"12px 0"}}>💳 Registrar pago {MESES[mes]}</button>
        </div>
        <div style={{ background:"#fff",borderRadius:14,padding:20,marginBottom:20,boxShadow:"0 2px 12px rgba(0,0,0,0.06)" }}>
          <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14 }}>
            <div style={{fontWeight:700,fontSize:15}}>👥 Grupo familiar ({familiaActual.residentes?.length||0} residente{familiaActual.residentes?.length!==1?"s":""})</div>
            <button onClick={()=>{setModalResidente(familiaActual.id);setEditResidente(null);setFormResidente({nombre:"",rut:"",observacion:""});}} style={sBtn("#0f4c81","#fff")}>+ Agregar residente</button>
          </div>
          {familiaActual.residentes?.length===0 ? (
            <div style={{color:"#94a3b8",textAlign:"center",padding:"20px 0",fontSize:13}}>Sin residentes registrados</div>
          ) : (
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
              <thead><tr style={{background:"#f8fafc"}}>{["Nombre","RUT","Observación",""].map(h=><th key={h} style={{padding:"8px 12px",textAlign:"left",color:"#64748b",fontWeight:600}}>{h}</th>)}</tr></thead>
              <tbody>{familiaActual.residentes.map(r=>(
        ...
        <div style={{ background:"#fff",borderRadius:14,padding:20,boxShadow:"0 2px 12px rgba(0,0,0,0.06)" }}>
          ...
          {familiaActual.pagos?.length===0 ? (
            ...
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
              <thead><tr style={{background:"#f8fafc"}}>{["Mes","Año","Monto","Forma","Observación",""].map(h=><th key={h} style={{padding:"8px 12px",textAlign:"left",color:"#64748b",fontWeight:600}}>{h}</th>)}</tr></thead>
              <tbody>{familiaActual.pagos.map(p=>(
                <tr key={p.id} style={{borderTop:"1px solid #f1f5f9"}}>
                  ...
                  <td style={{padding:"10px 12px"}}><button onClick={()=>eliminarPago(p.id)} style={sBtn("#fee2e2","#dc2626")}>🗑️</button></td>
                </tr>
              ))}</tbody>
            </table>
          )}
  ...
  return (
    <div className="main-container fade-in">
      ...
      <div style={{ display:"flex",gap:12,alignItems:"flex-end",marginBottom:16,flexWrap:"wrap" }}>
        <div><label style={sLabel}>Mes</label>
          <select value={mes} onChange={e=>setMes(Number(e.target.value))} style={sInput}>
            {MESES.slice(1).map((m,i)=><option key={i+1} value={i+1}>{m}</option>)}
          </select>
        </div>
        <div><label style={sLabel}>Año</label>
          <input type="number" value={anio} onChange={e=>setAnio(Number(e.target.value))} style={{...sInput,width:90}} />
        </div>
        <div><label style={sLabel}>Camión</label>
          <select value={camion} onChange={e=>{setCamion(e.target.value);setDiaFiltro("");}} style={sInput}>
            <option value="">Todos</option>{camiones.map(c=><option key={c} value={c}>{c}</option>)}
          </select>
        </div>
        <div><label style={sLabel}>Día</label>
          <select value={diaFiltro} onChange={e=>setDiaFiltro(e.target.value)} style={sInput} disabled={!camion}>
            <option value="">Todos los días</option>
            {(camion&&rutasDias[camion]?rutasDias[camion]:["LUNES","MARTES","MIERCOLES","JUEVES","VIERNES"]).map(d=><option key={d} value={d}>{d}</option>)}
          </select>
        </div>
        <div><label style={sLabel}>Buscar nombre</label>
          <input placeholder="Busca en los cargados..." value={busqueda} onChange={e=>setBusqueda(e.target.value)} style={{...sInput,minWidth:200}} />
        </div>
        ...
        <div style={{ marginLeft:"auto",display:"flex",gap:8,alignItems:"flex-end" }}>
          <div>
            ...
            <button onClick={exportarExcel} disabled={exportando||cargando} style={{...sBtn("#16a34a","#fff"),opacity:exportando?0.6:1}}>
              {exportando?"⏳...":"📤 Excel"}
            </button>
          </div>
          <div>
            <label style={{...sLabel,opacity:0}}>_</label>
            <button onClick={exportarPDF} disabled={exportando||cargando} style={{...sBtn("#0f4c81","#fff"),opacity:exportando?0.6:1}}>
              {exportando?"⏳...":"📄 PDF"}
            </button>
          </div>
        </div>
      ...
      <div style={{ background:"#fff",borderRadius:14,padding:"14px 20px",marginBottom:16,boxShadow:"0 2px 8px rgba(0,0,0,0.06)",display:"flex",alignItems:"center",gap:16,flexWrap:"wrap" }}>
        ...
        {editandoPrecio ? (
          ...
          <>
            <span style={{fontSize:20,fontWeight:700,color:"#0f4c81"}}>{precioUnitario>0?`$${Number(precioUnitario).toLocaleString()}`:"⚠️ Sin precio definido"}</span>
      ...
      {resumenKpis && (
        <div style={{ display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:12,marginBottom:16 }}>
          {[
            ...
            <div key={i} style={sKpi}><div style={{fontSize:11,color:"#64748b"}}>{k.label}</div><div style={{fontSize:18,fontWeight:700,color:k.color}}>{k.val}</div></div>
          ))}
        </div>
      )}
      ...
      <div style={{ background:"#fff",borderRadius:14,boxShadow:"0 2px 12px rgba(0,0,0,0.06)",overflow:"hidden" }}>
        <div style={{ padding:"12px 20px",borderBottom:"1px solid #f1f5f9",display:"flex",alignItems:"center",gap:12,flexWrap:"wrap" }}>
          <div style={{fontSize:13,color:"#64748b",fontWeight:600}}>
            {cargando?"⏳ Cargando primeras 50 familias...":`Mostrando ${familiasFiltradas.length} de ${totalCount} familias`}
          ...
          {(camion||diaFiltro||busqueda) && (
            <button onClick={()=>{setCamion("");setDiaFiltro("");setBusqueda("");}} style={{...sBtn("#f1f5f9","#64748b"),fontSize:11,padding:"4px 10px",marginLeft:"auto"}}>✕ Limpiar filtros</button>
          )}
        </div>
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            ...
            <tbody>
              {cargando && <tr><td colSpan={10} style={{padding:32,textAlign:"center"}}>
              ...
              {!cargando && familiasFiltradas.length===0 && (
                <tr><td colSpan={10} style={{padding:24,textAlign:"center",color:"#94a3b8"}}>
                  {busqueda?`Sin resultados para "${busqueda}"`:"Sin familias"}
                </td></tr>
              )}
            </tbody>
          </table>
        ...
        {!cargando && hayMas && (
          <div style={{padding:"16px 20px",borderTop:"1px solid #f1f5f9",textAlign:"center"}}>
            <button onClick={cargarMas} disabled={cargandoMas} style={{...sBtn("#0f4c81","#fff"),padding:"10px 32px",fontSize:14}}>
...
function ModalEditarFamilia({ formFamilia, setFormFamilia, guardarFamilia, cerrar }) {
  return (
    <div style={sOverlay}><div style={sModal}>
      ...
      <input placeholder="Nombre completo" value={formFamilia.nombre} onChange={e=>setFormFamilia(f=>({...f,nombre:e.target.value}))} style={{...sInput,width:"100%",marginBottom:10}} />
      <label style={sLabel}>Camión *</label>
      <select value={formFamilia.camion} onChange={e=>setFormFamilia(f=>({...f,camion:e.target.value}))} style={{...sInput,width:"100%",marginBottom:10}}>
        {["A1","A2","A3","A4","A5","M1","M2","M3"].map(c=><option key={c} value={c}>{c}</option>)}
      </select>
      <label style={sLabel}>Litros asignados *</label>
      <input type="number" placeholder="Ej: 2100" value={formFamilia.litros} onChange={e=>setFormFamilia(f=>({...f,litros:e.target.value}))} style={{...sInput,width:"100%",marginBottom:4}} />
      ...
      <input placeholder="Ej: 912345678" value={formFamilia.telefono} onChange={e=>setFormFamilia(f=>({...f,telefono:e.target.value}))} style={{...sInput,width:"100%",marginBottom:16}} />
...
function ModalPago({ modalPago, mes, anio, formPago, setFormPago, registrarPago, cerrar }) {
  return (
    <div style={sOverlay}><div style={sModal}>
      ...
      <input type="number" placeholder={`Sugerido: $${modalPago.deuda?.toLocaleString()}`} value={formPago.monto} onChange={e=>setFormPago(f=>({...f,monto:e.target.value}))} style={{...sInput,width:"100%",marginBottom:10}} />
      <label style={sLabel}>Forma de pago</label>
      <select value={formPago.forma_pago} onChange={e=>setFormPago(f=>({...f,forma_pago:e.target.value}))} style={{...sInput,width:"100%",marginBottom:10}}>
        {FORMAS_PAGO.map(fp=><option key={fp} value={fp}>{fp}</option>)}
      </select>
      <label style={sLabel}>Observación (opcional)</label>
      <input placeholder="Ej: Pago parcial..." value={formPago.observacion} onChange={e=>setFormPago(f=>({...f,observacion:e.target.value}))} style={{...sInput,width:"100%",marginBottom:16}} />
      <div style={{display:"flex",gap:10}}>
...
function ModalResidente({ editResidente, formResidente, setFormResidente, guardarResidente, cerrar }) {
  return (
    <div style={sOverlay}><div style={sModal}>
      ...
      <label style={sLabel}>Nombre *</label>
      <input placeholder="Nombre completo" value={formResidente.nombre} onChange={e=>setFormResidente(f=>({...f,nombre:e.target.value}))} style={{...sInput,width:"100%",marginBottom:10}} />
      <label style={sLabel}>RUT (opcional)</label>
      <input placeholder="Ej: 12.345.678-9" value={formResidente.rut} onChange={e=>setFormResidente(f=>({...f,rut:e.target.value}))} style={{...sInput,width:"100%",marginBottom:10}} />
      <label style={sLabel}>Observación (opcional)</label>
      <input placeholder="Ej: Adulto mayor..." value={formResidente.observacion} onChange={e=>setFormResidente(f=>({...f,observacion:e.target.value}))} style={{...sInput,width:"100%",marginBottom:16}} />
      <div style={{display:"flex",gap:10}}>
