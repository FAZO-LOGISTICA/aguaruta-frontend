...
export default function NoEntregadas() {
  ...
  const fetchData = async () => {
    try {
      ...
      const { data } = await axios.get(`${API_URL}/entregas`, { params });
  ...
  useEffect(() => { fetchData(); /* carga inicial */ }, []); // eslint-disable-line

  // Si no se selecciona estado, filtramos 0,2,3 en el front
  const filas = useMemo(() => {
    const base = estado === ""
      ? data.filter(r => [0,2,3].includes(Number(r.estado)))
      : data;
    return base.sort((a,b) => String(b.fecha).localeCompare(String(a.fecha)));
  }, [data, estado]);

  const camiones = useMemo(
    () => [...new Set(data.map(r => r.camion).filter(Boolean))].sort(),
    [data]
  );
  ...
  return (
    <div className="main-container fade-in">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 12, marginBottom: 12 }}>
        ...
        <div style={{ display:"flex", alignItems:"end", gap:8 }}>
          <button onClick={exportarExcel} disabled={!filas.length}>Exportar Excel</button>
          <button onClick={exportarPDF} disabled={!filas.length}>Exportar PDF</button>
        </div>
      </div>
